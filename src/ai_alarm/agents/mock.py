...
class ScriptedModel:
    def __init__(self, script: Callable[[], Mapping[str, Any]]):
        self.script = (
            script  # a function, so that the script can change while the system runs
        )
    ...
    def __call__(
        ...
        evidence = (
            request.get("evidence") or request.get("image") or request.get("video")
        )
        if isinstance(evidence, list):  # frames
            evidence = evidence[0]
        script = self.script()
        keys = (
            [f"{evidence}#{request['object_id']}", evidence]
