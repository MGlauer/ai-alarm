...
from ai_alarm.demos import (
    DEMOS,
    Demo,
    known_person_for_object,
    noise_category_for_evidence,
    scene_for_evidence,
)
...
class CctvAudioProcessor:
    def __init__(
        ...
        self.demo: Demo | None = (
            None  # the demo that was played last: the agents' mock models read its script
        )

    def play(
        self,
        name: str,
        emit: Callable[[SensorEventSignal], object],
        ...
        thread = threading.Thread(
            target=self._run, args=(demo, emit, time_scale), daemon=True
        )
        thread.start()
        if wait:
            thread.join()
    ...
    def idle_frame(
        ...
        if kind != "camera":
            return (
                render_audio_icon(sensor_id=sensor_id, area_name=area_name, at=at),
                "image/png",
            )
        night = _is_night(at)
    ...
    def frame(
        ...
        *,
        ...
        if spec is None:
            return (
                render_placeholder(sensor_id=sensor_id, area_name=area_name, at=at),
                "image/png",
            )
    ...
    def _with_sprite(self, obj: dict, evidence: str) -> dict:
        ...
        if obj.get("kind") == "person":
            person_id = known_person_for_object(
                evidence, obj.get("object_id", ""), self.demos
            )
            sprite = (
                IDENTITY_SPRITES.get(person_id)
                or ROLE_SPRITES.get(obj.get("role"))
                or "unknown_person"
            )
        elif obj.get("kind") == "animal":
            label = obj.get("label", "").lower()
            sprite = next(
                (name for key, name in ANIMAL_SPRITES.items() if key in label), None
            )
        else:
            sprite = None
    ...
    def _run(
        self, demo: Demo, emit: Callable[[SensorEventSignal], object], time_scale: float
    ) -> None:
        started = time.monotonic()
        for event in sorted(demo.events, key=lambda e: e.delay_s):
            time.sleep(
                max(0.0, event.delay_s * time_scale - (time.monotonic() - started))
            )
            try:
    ...
    def _signal(self, event) -> SensorEventSignal:
        with self.session_factory() as s:
            ...
            return SensorEventSignal(
                type="video_event" if is_video else "audio_event",
