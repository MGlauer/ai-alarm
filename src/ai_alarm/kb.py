...
from ai_alarm.db.models import (
    Area, AreaConnection, Contact, EntryPoint, PermissionRule, Person, PersonPresence, Role,
)
...
def _info(person: Person) -> PersonInfo:
    return PersonInfo(person.id, person.name, tuple(sorted(r.name for r in person.roles)), person.familiarity)
...
class KnowledgeBase:
    ...
    def reference_images(self) -> list[dict[str, Any]]:
        ...
        with self.session_factory() as s:
            persons = s.scalars(select(Person).where(Person.images.any()).order_by(Person.id))
            return [{"person_id": p.id, "images": [i.uri for i in p.images]} for p in persons]
    ...
    def record_detection(
        self, person_id: str, area_id: str, suspicion: float, now: datetime, predicted_role: str | None = None
    ) -> datetime:
        ...
        with self.session_factory() as s:
            ...
            open_presences = s.scalars(select(PersonPresence).where(
                PersonPresence.person_id == person_id, PersonPresence.left_at.is_(None))).all()
            ...
            if here is None:
                here = PersonPresence(person_id=person_id, area_id=area_id, entered_at=now, last_seen_at=now)
                s.add(here)
    ...
    def area_info(self, area_id: str) -> dict[str, Any]:
        ...
        with self.session_factory() as s:
            ...
            links = s.scalars(select(AreaConnection).where(
                or_(AreaConnection.area_a_id == area_id, AreaConnection.area_b_id == area_id)))
            neighbours = [link.area_b if link.area_a_id == area_id else link.area_a for link in links]
            return {
                "id": area.id, "name": area.name, "is_entryway": area.is_entryway, "is_drop_off": area.is_drop_off,
                "connected_to": [{"id": a.id, "name": a.name} for a in sorted(neighbours, key=lambda a: a.id)],
            }

    def has_open_entry_point(self) -> bool:
        with self.session_factory() as s:
            return s.scalars(select(EntryPoint.id).where(EntryPoint.is_open)).first() is not None

    def entry_permitted(
        self, area_id: str, roles: list[str], identified: bool, familiar: bool, invited: bool, stay_s: float
    ) -> bool:
            ...
            for rule in s.scalars(select(PermissionRule)):
                ...
                if rule.applies_to == "role":
                    if rule.role_name in roles and (identified or not rule.all_areas):
                        return True
                elif not familiar:  # rule for unfamiliar persons (also those that are not decidable)
                    limit_lifted = invited and rule.invitation_lifts_limit
                    if rule.max_duration_s is None or stay_s <= rule.max_duration_s or limit_lifted:
                        return True
            return False
    ...
    def people_present(self, area_id: str | None = None) -> list[PersonInfo]:
        with self.session_factory() as s:
            query = select(Person).join(PersonPresence).where(PersonPresence.left_at.is_(None))
    ...
    def close_presences(self, area_id: str, now: datetime) -> None:
        with self.session_factory() as s:
            for p in s.scalars(select(PersonPresence).where(
    ...
    def recipients(self) -> list[Recipient]:
        ...
        with self.session_factory() as s:
            present = select(PersonPresence.person_id).where(PersonPresence.left_at.is_(None))
            contacts = s.scalars(select(Contact).where(
                or_(Contact.is_primary, Contact.person_id.in_(present))).order_by(Contact.id))
