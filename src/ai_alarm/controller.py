...
class PersonAssessment(Part):
    ...
    name: str | None = (
        None  # display name from the knowledge base, for identity "known" only (may still be unset)
    )
    ...
    invited: bool = (
        False  # invited or accompanied by a family member in the immediate past
    )
    bbox: BBox | None = (
        None  # where they are in the evidence of the event this assessment is based on
    )
    ...
    def _known_persons_have_an_id(self):
        if (self.identity == "known") != (self.person_id is not None):
            raise ValueError(
                "person_id is required for, and only for, the identity 'known'"
            )
        return self
...
class AnimalAssessment(Part):
    ...
    bbox: BBox | None = (
        None  # where it is in the evidence of the event this assessment is based on
    )
...
class SituationSummarySignal(SituationSignal):
    ...
    weather_suspicion: Score = (
        0.0  # of a weather event that does not match the forecast
    )
...
class ControllerConfig:
    warning_threshold: float = (
        0.4  # a person that exceeds this suspicion triggers a warning ...
    )
    alarm_threshold: float = 0.85  # ... and one that exceeds this an alarm
    familiar_threshold: float = (
        0.5  # a known person who is at least this familiarity is familiar
    )
    answer_timeout_s: float = (
        600  # after this, the fallback policy answers an open warning
    )
    similar_alarm_window_s: float = (
        300  # no second alarm with the same cause and area within this time
    )
    unpermitted_grace_s: float = (
        60  # an unpermitted entry that lasts this long becomes an alarm
    )
    obstruction_prolonged_s: float = (
        120  # an implausible obstruction that lasts this long is reported
    )
    default_suspicion: float = (
        0.5  # suspicion of a warning that does not come from a situation summary
    )
    plausible_obstruction_weather: frozenset[str] = frozenset(
        {"fog", "snowfall", "rain"}
    )
    harmless_role_mismatch: float = (
        0.3  # a person that blocks the view is harmless up to this role mismatch ...
    )
    suspicious_intents: frozenset[str] = frozenset(
        {"suspicious_activity", "unknown_activity"}
    )  # ... and unless these
    family_role: str = "family"
    dangerous_animal_threshold: float = (
        0.9  # danger score from which an animal is high-danger (1 = bear)
    )
    animal_alarm_if_people_present: bool = (
        False  # optional condition for the "dangerous animal" alarm
    )
    animal_alarm_window: tuple[time, time] | None = (
        None  # optional daily time frame for the same
    )
...
class Controller:
    ...
    def handle_sensor_event(self, signal: SensorEventSignal) -> str:
        ...
        with self._lock, self.session_factory() as s:
            ...
            situation = s.scalars(
                select(Situation).where(
                    Situation.area_id == signal.area_id, Situation.status != "resolved"
                )
            ).first()
            ...
            s.add(
                SensorEvent(
                    ...
                    kind=signal.type.removesuffix("_event"),
        ...
        (self.interpreters.start if is_new else self.interpreters.forward)(
            situation_id, event
        )
        return situation_id
    ...
    def handle_situation_resolved(self, signal: SituationResolvedSignal) -> None:
        with self._lock:
            ...
            with self.session_factory() as s:
                ...
                s.flush()
                unresolved = s.scalars(
                    select(Situation.id).where(
                        Situation.scenario_id == situation.scenario_id,
                        Situation.status != "resolved",
                    )
                ).first()
                if (
                    unresolved is None
                ):  # the scenario is resolved when its last situation is
    ...
    def handle_situation_summary(self, signal: SituationSummarySignal) -> None:
        with self._lock:
            ...
            if (
                signal.unclear_situation
                and signal.threat_score >= self.config.warning_threshold
            ):
                self._warn(
                    signal.situation_id,
                    signal.area_id,
    ...
    def _store_summary(
        self, signal: SituationSummarySignal, now: datetime
    ) -> dict[str, float]:
        ...
        with self.session_factory() as s:
            ...
            s.add(
                AggregatedSummary(
                    ...
                    data={
                        "situations": {x.situation_id: x.threat_score for x in latest},
                        "persons": combined,
                    },
                )
            )
    ...
    def _latest_summaries(s: Session, scenario_id: str) -> list[SituationSummarySignal]:
        ...
        newest = select(func.max(SituationSummary.id)).group_by(
            SituationSummary.situation_id
        )
        rows = s.scalars(
            select(SituationSummary)
            .join(Situation)
            .where(
                SituationSummary.id.in_(newest),
                Situation.scenario_id == scenario_id,
                Situation.status != "resolved",
            )
        )
    ...
    def _record_persons(
        ...
        for p in signal.persons:
            ...
            if p.identity != "not_decidable" or p.cause == "low confidence":
                ...
                entered = self.kb.record_detection(
                    key, signal.area_id, combined[key], now, predicted
                )
            else:
                entered = started
    ...
    def _check_suspicion(
        self, signal: SituationSummarySignal, combined: dict[str, float]
    ) -> None:
        """A person, or the weather, that exceeds the suspicion thresholds triggers a warning or an alarm."""
        top_person = max(
            (
                combined[self._person_key(signal.situation_id, p)]
                for p in signal.persons
            ),
            default=0.0,
        )
        for suspicion, cause in (
            (top_person, "suspicious person"),
            (signal.weather_suspicion, "suspicious weather"),
        ):
            if suspicion > self.config.alarm_threshold:
                self._alarm(
                    signal.situation_id, signal.area_id, "strong suspicion", "rule"
                )
            elif suspicion > self.config.warning_threshold:
                self._warn(
                    signal.situation_id, signal.area_id, cause, signal.threat_score
                )

    def _check_permissions(
        self,
        signal: SituationSummarySignal,
        combined: dict[str, float],
        ...
        for p in signal.persons:
            ...
            roles = (
                list(info.roles)
                if info
                else [p.predicted_role] if p.predicted_role else []
            )
            familiar = (
                info is not None and info.familiarity >= self.config.familiar_threshold
            )
            if self.kb.entry_permitted(
                area_id,
                roles,
                info is not None,
                familiar,
    ...
    def _entry_is_prolonged(self, situation_id: str, now: datetime) -> bool:
        ...
        with self.session_factory() as s:
            ...
            warning = s.scalars(
                select(SituationWarning)
                .join(Situation)
                .where(
                    Situation.scenario_id == scenario_id,
                    SituationWarning.cause == "unpermitted entry",
                )
            ).first()
            return (
                warning is not None
                and warning.status != "dismissed"
                and (now - warning.created_at).total_seconds()
                >= self.config.unpermitted_grace_s
            )

    def _check_animals(self, signal: SituationSummarySignal, now: datetime) -> None:
        if not any(
            a.danger >= self.config.dangerous_animal_threshold for a in signal.animals
        ):
            return
        ...
        if (
            self.kb.has_open_entry_point()
            or (self.config.animal_alarm_if_people_present and self.kb.people_present())
            or (
                window
                and in_window(now.astimezone(self.comm.config.tz).time(), *window)
            )
        ):
            self._alarm(signal.situation_id, signal.area_id, "dangerous animal", "rule")
        else:
    ...
    def handle_obscured(self, signal: ObscuredSignal) -> None:
        ...
        with self._lock:
            ...
            if self._plausible_by_weather(sid, now):
                self._log(
                    sid, "obscured_plausible", "the weather explains the obstruction"
                )
                return
            if self._plausible_by_person(signal):
                self._log(
                    sid,
                    "obscured_plausible",
                    "a harmless person explains the obstruction",
                )
                return
            ...
            audio = self._call(
                sid, "audio source", self.audio.latest_audio, area_id, now
            )
            if audio is None:
                return
            noise = self._ask(
            ...
            family_around = any(
                self.config.family_role in p.roles
                for p in self.kb.people_present(area_id)
            )
            if (
                noise is not None
                and noise.category == "human_activity"
                and not family_around
            ):
    ...
    def _note_obscured(self, situation_id: str, now: datetime) -> datetime:
        ...
        with self.session_factory() as s:
            ...
            s.commit()
            return s.scalars(
                select(func.min(LogEntry.created_at)).where(
                    LogEntry.situation_id == situation_id, LogEntry.kind == "obscured"
                )
            ).one()

    def _plausible_by_weather(self, situation_id: str, now: datetime) -> bool:
        conditions = self._call(
            situation_id, "weather forecast", self.weather.conditions_at, now
        )
        return bool(
            conditions and set(conditions) & self.config.plausible_obstruction_weather
        )
    ...
    def check_timeouts(self) -> None:
        ...
        with self.session_factory() as s:
            due = list(
                s.scalars(
                    select(SituationWarning.id).where(
                        SituationWarning.status == "open",
                        SituationWarning.answer_deadline <= self.clock(),
                    )
                )
            )
    ...
    def _fallback(self, warning_id: str) -> None:
        ...
        self._answer(
            warning_id,
            "elevated" if colour == "orange" else "dismissed",
            "fallback_policy",
        )
        ...
        with self._lock:
            ...
            with self.session_factory() as s:
                ...
                if warning.status != "open":  # answered already, e.g. by the timeout
                    return
                warning.status, warning.resolved_by, warning.resolved_at = (
                    status,
                    resolved_by,
                    now,
                )
                s.add(
                    LogEntry(
                        created_at=now,
                        situation_id=warning.situation_id,
                        kind=f"warning_{status}",
                        message=f"{warning.cause}: {status} ({resolved_by})",
                        data={"warning_id": warning.id},
                    )
                )
                situation_id, area_id, cause = (
                    warning.situation_id,
                    warning.area_id,
                    warning.cause,
                )
                s.commit()
            if status == "elevated":
                origin = (
                    "user_elevation" if resolved_by == "user" else "fallback_policy"
                )
    ...
    def _warn(
        ...
        deliveries = self.comm.send_warning(
            warning_id, cause, area_id, suspicion, colour
        )
        with self.session_factory() as s:
            s.add(
                SituationWarning(
                    ...
                    created_at=now,
                    answer_deadline=now
                    + timedelta(seconds=self.config.answer_timeout_s),
                )
            )
            s.flush()
            ...
            s.commit()
        if not any(
            d.sent for d in deliveries
        ):  # nobody could be reached: the fallback policy applies right away
            self._fallback(warning_id)
        return warning_id
    ...
    def _alarm(
        ...
        with self.session_factory() as s:
            ...
            similar = s.scalars(
                select(SituationAlarm).where(
                    SituationAlarm.cause == cause,
                    SituationAlarm.area_id == area_id,
                    SituationAlarm.created_at
                    >= now - timedelta(seconds=self.config.similar_alarm_window_s),
                )
            ).first()
            if similar is not None:  # no flooding for distinct situations
                s.add(
                    IdempotencyKey(
                        key=key,
                        kind="alarm",
                        result={"alarm_id": similar.id, "suppressed": True},
                        created_at=now,
                    )
                )
                s.add(
        ...
        with self.session_factory() as s:
            ...
            s.add(
                IdempotencyKey(
                    key=key, kind="alarm", result={"alarm_id": alarm_id}, created_at=now
                )
            )
            s.add(
                self._log_entry(
    ...
    def _speak(self, situation_id: str, cause: str, text: str) -> None:
        ...
        with self.session_factory() as s:
            s.add(
                IdempotencyKey(
                    key=key, kind="speak", result={"text": text}, created_at=now
                )
            )
    ...
    @staticmethod
    def _notifications(
        deliveries: list[Delivery], now: datetime, **subject: str
    ) -> list[Notification]:
        return [
            Notification(
                contact_id=d.contact_id,
                message=d.message,
                status="sent" if d.sent else "failed",
                created_at=now,
                **subject,
            )
            for d in deliveries
    ...
    def _threat(self, situation_id: str) -> float:
        ...
        with self.session_factory() as s:
            score = s.scalars(
                select(SituationSummary.threat_score)
                .where(SituationSummary.situation_id == situation_id)
                .order_by(SituationSummary.id.desc())
            ).first()
