...
class WeatherForecastFetcher:
    def __init__(
        self, fetch: Callable[[], list[ForecastEntry]], clock: Callable[[], datetime] = utcnow, max_age_s: float = 3600
    ):
        self.fetch = fetch
        self.clock = clock
        ...
        self._asked_at: datetime | None = None  # when the service was asked last, successfully or not
    ...
    def get_forecast(self, at: datetime) -> Forecast:
        ...
        with self._lock:
            now = self.clock()
            from_cache = True
            if self._asked_at is None or (now - self._asked_at).total_seconds() >= self.max_age_s:
                self._asked_at = now
                try:
                    self._entries, self._fetched_at, from_cache = list(self.fetch()), now, False
                except Exception:
                    if self._entries is None:
                        self._asked_at = None  # nothing to fall back on: ask again next time
                        raise
            entries = [e for e in self._entries if e.valid_from <= at < e.valid_to]
            return Forecast(entries=entries, fetched_at=self._fetched_at, from_cache=from_cache)
