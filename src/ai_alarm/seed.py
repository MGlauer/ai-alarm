...
def seed_demo_house(factory: Callable[[], Session]) -> None:
    ...
    family, delivery, gardener = (
        Role(name="family"),
        Role(name="delivery person"),
        Role(name="gardener"),
    )
    entry, garden, house = (
        Area(id="entry", name="Entryway", is_entryway=True),
        Area(id="garden", name="Garden"),
    ...
    with factory() as s:
        s.add_all(
            [
                ...
                EntryPoint(
                    id="door", name="Front door", area_id="entry", is_open=False
                ),
                Sensor(
                    id="cam_entry",
                    kind="camera",
                ...
                Person(
                    ...
                    roles=[family],
                    images=[
                        PersonImage(uri="anna_1.png"),
                        PersonImage(uri="anna_2.png"),
                    ],
                ),
                Person(
                    id="gustav",
                ...
                PermissionRule(
                    description="family may enter all areas",
                    applies_to="role",
                    role_name="family",
                    all_areas=True,
                ),
                PermissionRule(
                    description="delivery people: entryway",
                    applies_to="role",
                    role_name="delivery person",
                    areas=[entry],
                ),
                PermissionRule(
                    description="gardeners: garden and entryway",
                    applies_to="role",
                    role_name="gardener",
                    areas=[garden, entry],
                ),
                PermissionRule(
                    description="unfamiliar persons: entryway for two minutes, unlimited if invited",
                    applies_to="unfamiliar",
                    areas=[entry],
                    max_duration_s=120,
                    invitation_lifts_limit=True,
                ),
            ]
        )
        s.commit()
