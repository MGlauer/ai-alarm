...
class CommConfig:
    orange_threshold: float = (
        0.75  # a warning above this suspicion is orange, otherwise yellow
    )
    night_orange_threshold: float = (
        0.5  # lowered at night: users answer less often, so unanswered ones are elevated
    )
...
class CommunicationUnit:
    ...
    def is_night(self, now: datetime) -> bool:
        return in_window(
            now.astimezone(self.config.tz).time(),
    ...
    def colour(self, suspicion: float, now: datetime) -> str:
        threshold = (
            self.config.night_orange_threshold
            if self.is_night(now)
            else self.config.orange_threshold
        )
        return "orange" if suspicion > threshold else "yellow"

    def send_warning(
        self, warning_id: str, cause: str, area_id: str, suspicion: float, colour: str
    ) -> list[Delivery]:
        text = TEMPLATES[cause].format(area=self.kb.area_info(area_id)["name"])
        return self._send(
            f"{colour.upper()} WARNING ({suspicion:.2f}): {text} Elevate or dismiss it in the app "
    ...
    def _send(self, message: str) -> list[Delivery]:
        ...
        for recipient in self.kb.recipients():
            try:
                ...
                Exception
            ):  # noqa: BLE001 - one unreachable contact must not stop the others
                sent = False
